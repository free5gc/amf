...
func action(cliCtx *cli.Context) error {
	...
	appStart := func(a *service.AmfApp) {
		...
		if profileTmp, err1 := consumer.BuildNFInstance(a.Context()); err1 != nil {
			logger.InitLog.Error("Build AMF Profile Error")
		} else {
			profile = profileTmp
		}
		_, nfId, err_reg := consumer.SendRegisterNFInstance(a.Context().NrfUri, a.Context().NfId, profile)
		if err_reg != nil {
	...
	appStop := func(a *service.AmfApp) {
		// deregister with NRF
		problemDetails, err_deg := consumer.SendDeregisterNFInstance()
